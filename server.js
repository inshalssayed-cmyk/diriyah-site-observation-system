...
const sharedStatusPassword =
  process.env.STATUS_PASSWORD ||
  "1353";

const PROJECTS = Object.freeze({
  MRES: Object.freeze({
    code: "MRES",
    name: "Diriyah - Media Residential",
    homeTitle: "MEDIA RESIDENTIAL PROJECT",
    excelTitle: "DIRIYAH MEDIA RESIDENTIAL PROJECT",
    observationPrefix: "MRES",
    fileSlug: "media-residential",
    storageFolder: "mres",
    statusPassword: sharedStatusPassword
  }),

  MSO: Object.freeze({
    code: "MSO",
    name: "Diriyah - Media South Office",
    homeTitle: "MEDIA SOUTH OFFICE PROJECT",
    excelTitle: "DIRIYAH MEDIA SOUTH OFFICE PROJECT",
    observationPrefix: "MSO",
    fileSlug: "media-south-office",
    storageFolder: "mso",
    statusPassword:
      process.env.MSO_STATUS_PASSWORD ||
      sharedStatusPassword
  }),

  ONE_HOTEL: Object.freeze({
    code: "ONE_HOTEL",
    name: "Diriyah - One Hotel",
    homeTitle: "ONE HOTEL PROJECT",
    excelTitle: "DIRIYAH ONE HOTEL PROJECT",
    observationPrefix: "OH",
    fileSlug: "one-hotel",
    storageFolder: "one-hotel",
    statusPassword:
      process.env.ONE_HOTEL_STATUS_PASSWORD ||
      "9528"
  })
});
...
async function initializeDatabase() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS
      observation_project_daily_counters (
    ...
    CREATE INDEX IF NOT EXISTS
      ...
      ON observations(project_code);

    CREATE INDEX IF NOT EXISTS
      observations_project_status_idx
      ON observations(
        project_code,
        status
      );
...
/* =====================================================
   PROJECT SELECTION AND PASSWORD PROTECTION
===================================================== */

function getProjectByCode(value) {
  ...
  return PROJECTS[projectCode] || null;
}
...
function requireProject(
  ...
  const project = getProjectByCode(
    request.get("x-project-code") ||
      request.body?.projectCode ||
...
function requireStatusPassword(
  ...
  if (
    suppliedPassword !==
    request.project.statusPassword
  ) {
    ...
    try {
      ...
      uploadedIssueImages =
        await uploadImagesToCloudinary(
          ...
          `diriyah-observations/${project.storageFolder}/issues/${observationDate}`
        );
      ...
      const observationNumber =
        `${project.observationPrefix}-${compactDate}-${String(
          dailySequence
        ).padStart(3, "0")}`;
    ...
    try {
      ...
      uploadedCloseoutImages =
        await uploadImagesToCloudinary(
          ...
          `diriyah-observations/${request.project.storageFolder}/closeouts/${observationId}`
        );
    ...
    try {
      ...
      const uploadedIssueImages =
        await uploadImagesToCloudinary(
          ...
          `diriyah-observations/${request.project.storageFolder}/issues/${normalizeDateValue(
  ...
  async (
    ...
    try {
      ...
      response.setHeader(
        ...
        `attachment; filename="diriyah-${request.project.fileSlug}-observation-status-${downloadDate}.xlsx"`
      );
  ...
  async (
    ...
    try {
      ...
      response.setHeader(
        ...
        `attachment; filename="diriyah-${request.project.fileSlug}-observations-${downloadDate}.csv"`
      );
...
initializeDatabase()
  .then(() => {
    app.listen(
      ...
      () => {
        console.log(
          `Diriyah Multi-Project Observation System V4.0.0 is running on port ${port}`
        );
